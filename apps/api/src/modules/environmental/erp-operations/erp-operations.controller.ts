...
export async function remove(req: Request, res: Response) {
  const row = await svc.deleteErpOperation(req.user!.orgId, req.params['id'] as string);
  return row ? ok(res, row) : notFound(res, 'Erp Operation');
}

export async function update(req: Request, res: Response) {
  const row = await svc.updateErpOperation(req.user!.orgId, req.params['id'] as string, req.body as UpdateErpOperationBody);
  return row ? ok(res, row) : notFound(res, 'Erp Operation');
}
